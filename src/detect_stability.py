...
model_names = ["lstm", "nbeats", "tft", "nhits", "dlinear", "nlinear", "tsmixer"]
#model_name = "tft"


delay_time = [5.823, 7.055, 5.509, 5.474, 5.776, 4.003, 3.976, 6.741]

# Define a custom colormap with your desired sequence of colors
custom_colors = ["#008000", "#6B4F1C", "#FFC0CB", "#0000FF", "#FFD700","#FF8C00" , "#FF6347", "#800080", "#D3D3D3" ,"#00FFFF" ]
custom_markers = ['o', 's', '^', 'D', 'P', 'X', '*']  # Circle, square, triangle, diamond, plus-filled, X, star

#custom_colors = ["#008000", "#6B4F1C", "#0000FF","#FF8C00" , "#FF6347", "#D3D3D3" ,"#FFC0CB","#FFC0CB",  "#FFD700", "#800080" ]
...
for model_name in model_names:
    #ref_data = pd.to_numeric(eval_results[model_name]["equi"], errors='coerce')
    ref_data = []
    for i, item in enumerate(final_time_diff[f"{model_name}_time_diff"].to_list()):
        ref_data.append(delay_time[i] + item+1)
    model_dict[model_name] = ref_data

print(model_dict)
# Sample DataFrame
data = {
    "Dataset": [26,27,28,29,30,31,32,33],
    "Delay": [
        5.823+1, 7.055+1, 5.509+1, 5.474+1, 5.776+1,
        4.003+1, 3.976+1, 6.741+1
    ],
    "Additional_Data": ref_data
}
df = pd.DataFrame(data)

# Plotting the bar chart
ax = df.plot.bar(x="Dataset", y="Delay", legend=False, color='#1f77b4', alpha=0.4,figsize=(12, 8))

# Get the x-tick positions for the bars
x_positions = range(len(df["Dataset"]))

# Add a red horizontal line at y=1
ax.axhline(y=1, color='red', linestyle='--', linewidth=2)

for i, model_name in enumerate(model_names):
    plt.scatter(x_positions, model_dict[model_name], color=custom_colors[i], alpha=1, label=model_name, zorder=5, s=200, marker=custom_markers[i])

# Customizing the plot
plt.xlabel("Datasets", fontsize=24)
plt.ylabel("Delay (hour)", fontsize=24)
#plt.title("Bar Chart: Datasets vs Delay")
...
#plt.subplots_adjust(left=0.3)  # Make space for the legend

#plt.legend(loc='center right', bbox_to_anchor=(-0.1, 0.5), fontsize=18)
# Place the legend at the bottom in a horizontal line
plt.legend(
    ["start forecast", "LSTM", "NBeats", "TFT", "N-Hits", "DLinear", "NLinear", "TSMixer"],  # Unpack the handles and labels
    loc="lower center",     # Position of the legend
    bbox_to_anchor=(0.5, -0.55),  # Fine-tune position below the plots
    fontsize=24,
    ncol=3                 # Spread the legend items into 3 columns
)

#[left, bottom, right, top]
#plt.tight_layout(rect=[0.5, 1, 0.5, 0])  # Adjust layout to include the title

# Adjust layout
#plt.tight_layout(rect=[0.2, 0.03, 1, 0.95])  # Adjust layout to include the title
#plt.subplots_adjust(left=0.3)  # Make space for the legend
plt.tight_layout()
